...
sealed class JsonField<out T : Any> {
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw OpenlayerInvalidDataException("`$name` is not set")
            is JsonNull -> throw OpenlayerInvalidDataException("`$name` is null")
            else -> throw OpenlayerInvalidDataException("`$name` is invalid, received $this")
        }
