    ...
    private constructor(
        ...
        class Builder internal constructor() {
            ...
            internal fun from(item: Item) = apply {
                ...
                name = item.name
                number = item.number
                originProjectVersionId = item.originProjectVersionId
                subtype = item.subtype
                suggested = item.suggested
                thresholds = item.thresholds.map { it.toMutableList() }
                type = item.type
                archived = item.archived
                defaultToAllPipelines = item.defaultToAllPipelines
                delayWindow = item.delayWindow
                evaluationWindow = item.evaluationWindow
                excludePipelines = item.excludePipelines.map { it.toMutableList() }
                includeHistoricalData = item.includeHistoricalData
                includePipelines = item.includePipelines.map { it.toMutableList() }
                usesMlModel = item.usesMlModel
                usesProductionData = item.usesProductionData
                usesReferenceDataset = item.usesReferenceDataset
                usesTrainingDataset = item.usesTrainingDataset
                usesValidationDataset = item.usesValidationDataset
